...
var replayBtn = null,homeBtn = null,sndBtn,nosndBtn, progress = null,totalLoaded = 0;
var everyGameTime = {second:0,minute:0,hour:0},//向后台传送的时间对象
  gameScores = {},//往服务器传数据的对象
  reward = {};//奖励对象
var gameName = null, gameTimer = null,//计时函数
...
function handleMainLoad() {
  ...
  //重玩按钮
  replayBtn = new createjs.Bitmap(load_preload.images["replay-btn"]);
  ...
  //返回主页
  homeBtn = new createjs.Bitmap(load_preload.images["home-btn"]);
  ...
  //背景音乐按钮
  sndBtn = new createjs.Bitmap(load_preload.images["snd-btn"]);
  ...
  nosndBtn = new createjs.Bitmap(load_preload.images["nosnd-btn"]);
...
function gotoHome(url){
  ...
  var cancle = new createjs.Bitmap(load_preload.getResult("cancel"));
  cancle.x = 640;
  cancle.y = 210;
    ...
    $.ajax({
      ...
      success:function(result){
        if(result.success){
          ...
          show.gameReward(reward,gameAllConfig.rewardPosition,gameOverAnimation)
        }
      }
    })
  }
